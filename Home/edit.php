...
if (!$_SESSION['emailAddress']) {
    ...
    $mysql = connectBlog();
    $postId = $_GET['postId'];
    $queryAutoFill = mysqli_query($mysql, "SELECT post_title, post_blog FROM forum WHERE post_id = '$postId'");
   
    while ($row = mysqli_fetch_array($queryAutoFill, MYSQLI_ASSOC)) {
        //Variables will fill in the form to be edited.
    ...
    if (isset($_POST['submit'])) {
        ...
        if ((empty($postBlog)) || (empty($postTitle))) {
            ...
            $query = mysqli_query($mysql, "UPDATE forum SET user_id = '$userId', post_title = '$postTitle', post_blog = '$postBlog', post_date = '$date', post_time = '$time' WHERE post_id = '$postId'");
